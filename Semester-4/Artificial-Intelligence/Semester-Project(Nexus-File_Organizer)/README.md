# Nexus File Organizer – AI Semester Project

**Nexus File Organizer** is a desktop application developed in **Python** that automatically organizes files inside a selected folder based on different criteria such as file type, date, alphabetical order, or file size.

The application provides a **graphical user interface (GUI)** that helps users manage and organize large collections of files efficiently.

---

## Features

* Organize files by **file type** (images, documents, videos, etc.)
* Organize files by **date**
* Organize files **alphabetically**
* Organize files by **file size**
* Automatically creates folders for categorized files
* Simple and user-friendly GUI
* Visualization of file statistics

---

## Technologies Used

* **Python**
* **CustomTkinter** (GUI framework)
* **Pandas**
* **Matplotlib**
* **Pillow (PIL)**

---

## Project Structure

Semester-Project (Nexus-File_Organizer)

* NexusFile_Pro.py
* NexusFile_Pro.spec
* NexusFile Pro Advanced.exe
* README.md

---

## Project Type

**Semester Project**
BS Computer Science – Artificial Intelligence Course

---

## Authors

**Rifaq Ajmal**
**Muhammad Uzair**
